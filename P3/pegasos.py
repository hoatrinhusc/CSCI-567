...
def objective_function(X, y, w, lamb):
    ...
    sq = (np.linalg.norm(w))**2
    temp = np.multiply(y, np.dot(X,w).T)
    A = np.maximum(0, 1-temp)
    obj_value = np.mean(A) + sq*lamb/2
...
def pegasos_train(Xtrain, ytrain, w, lamb, k, max_iterations):
    ...
    for iter in range(1, max_iterations + 1):
        ...
        # you need to fill in your solution here
        broad = np.size(A_t, axis = 0)
        Xt = Xtrain[A_t]
        yt = ytrain[A_t]
        temp = np.multiply(yt, np.dot(Xt,w).T)
        bool_idx = (temp <1)
        bool_idx =bool_idx.reshape(broad,)
        yt = yt[bool_idx]
        Xt = Xt[bool_idx]
        ...
        rate = 1/(lamb*iter)
        w *= 1-rate*lamb
        w += (rate/k) * np.dot(Xt.T,yt)
        project = 1/(np.sqrt(lamb) * np.linalg.norm(w))
        w *= min(1,project)
...
def pegasos_test(Xtest, ytest, w_l):
    ...
    y = np.array(ytest)
    pred = np.dot(X,w_l)
    ypred = [-1 if yk < 0 else 1 for yk in pred]
    
    error = np.count_nonzero(ypred-y) 
    
    test_acc = 1- error/y.shape[0]
