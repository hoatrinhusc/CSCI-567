...
def multinomial_train(X, y, C, 
    ...
    for i in range(max_iterations):
        ...
        Pnorm = Pnorm - yhot
        GD_w = np.dot(Pnorm,X)/N
        GD_b = np.sum(Pnorm,axis = 1)/N
        w -= step_size * GD_w
        b -= step_size * GD_b
...
def OVR_predict(X, w, b):
    ...
    TODO: add your code here
    """
    preds = multinomial_predict(X, w, b)
